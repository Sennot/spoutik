...
checks = {
    ...
    "Spout documented default FBO": r"SendFbo\(0, 0, 0, invert\)",
    "Spout CPU fallback rejected": r"if \(m_spout->GetCPU\(\)\)[\s\S]*m_cpuFallbackRejected = true",
    "Spout GLDX is diagnostic only": r"legacy NVIDIA NV_DX_interop2[\s\S]*false GetGLDX\(\) does NOT mean",
    ...
    "mirror all scheduler callbacks removed": r"unscheduleAllForTarget\(m_mirror\)",
    "mirror scheduler quiesced before release": r"if \(m_mirror\) \{[\s\S]*quiesceMirrorScheduler\(\);[\s\S]*m_mirror->release\(\)",
    "real-layer root hooks are VeryLate": r"PlayLayer::init[\s\S]*Priority::VeryLate[\s\S]*GJBaseGameLayer::update[\s\S]*Priority::VeryLate",
    "mirror side-effect guards are Last": r"PlayLayer::prepareMusic[\s\S]*PlayLayer::addObject[\s\S]*Priority::Last",
